...
class MainWindow(tkinter.Frame):
    ...
    def __init__(self, parent):
        ...
        principalScale = tkinter.Scale(self, variable=self.principal,command = self.updateU1, from_ =100, to=10000000,resolution = 100, orient = tkinter.HORIZONTAL)
        rateLabel = tkinter.Label(self, text="Rate %", underline=0, anchor=tkinter.W)
        rateScale = tkinter.Scale(self, variable=self.rate,command = self.updateU1, from_ = 1, to = 100,resolution = 0.25, digits = 5, orient = tkinter.HORIZONTAL)
        yearsLabel = tkinter.Label(self, text="Years", underline=0, anchor=tkinter.W)
        yearsScale = tkinter.Scale(self, variable=self.years,command = self.updateU1, from_ = 1, to = 50,orient = tkinter.HORIZONTAL)
        amountLabel = tkinter.Label(self, text="Amount S:", anchor=tkinter.W)
